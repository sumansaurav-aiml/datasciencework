# -*- coding: utf-8 -*-
"""export_col_filter_prod.py

This module exports the Collabortive Model Data from CSV to Database table
...
class ExportColFilterProd:
    """exports the Collabortive Model Data from CSV to Database table.
    ...
    def __init__(self, jobid, root_file_path):
        ...
        self._IN_FILE_PATH = './'+root_file_path+'/data/jobid' + str(self._jobid) + '/predictionfiles/TOP_COL_FILTER_REC_PROD_FOR_PORTALS_USERS.CSV'

    def export_colfilter_rec(self, conn_string):
    ...
    def _modify_columns(self, df):
        ...
        """
        # modify the column as per table in database
    ...
    def _export_data_in_db(self, conn, engine, df):
        ...
        """
        self._logger.info("Truncating table COL_FILTER_REC_PORTAL_USER!")
        conn.execute("""TRUNCATE TABLE COL_FILTER_REC_PORTAL_USER""")
        self._logger.info("Inserting data into COL_FILTER_REC_PORTAL_USER!")
        df.to_sql('COL_FILTER_REC_PORTAL_USER', con=engine, if_exists='append', chunksize=1000, index=False)
        self._logger.info("Inserting data into COLFILTER_REC_PORTAL_USER_HIST!")
        conn.execute("""INSERT INTO COLFILTER_REC_PORTAL_USER_HIST (USER_ID,ITEM_ID,RATING,PORTAL,AIML_JOB_RUN_ID)
