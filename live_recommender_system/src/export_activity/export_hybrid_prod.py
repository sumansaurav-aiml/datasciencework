# -*- coding: utf-8 -*-
"""export_hybrid_prod.py

This module exports the Hybrid Recommended Data from CSV to Database table
...
class ExportHybridProd:
    """exports the Hybrid Recommended Data from CSV to Database table.
    ...
    def __init__(self, jobid, root_file_path):
        ...
        self._IN_FILE_PATH = './'+root_file_path+'/data/jobid' + str(self._jobid) + '/predictionfiles/TOP_HYBRID_REC_FOR_PORTALS_USERS.CSV'

    def export_hybrid_rec(self, conn_string):
        ...
        if df.shape[0] == 0:
            raise Exception("Data not found in the csv file: TOP_HYBRID_REC_FOR_PORTALS_USERS.csv")
    ...
    def _export_data_in_db(self, conn, engine, df):
        ...
        """
        self._logger.info("Truncating table HYBRID_RECFOR_PORTAL_USER!")
        conn.execute("""TRUNCATE TABLE HYBRID_RECFOR_PORTAL_USER""")
        self._logger.info("Inserting data into HYBRID_RECFOR_PORTAL_USER!")
        df.to_sql('HYBRID_RECFOR_PORTAL_USER', con=engine, if_exists='append', chunksize=1000, index=False)
        self._logger.info("Inserting data into HYBRID_RECFOR_PORTAL_USER_HIST!")
        conn.execute("""INSERT INTO HYBRID_RECFOR_PORTAL_USER_HIST (USER_ID,ITEM_ID,SORT_ORDER,PORTAL,MODEL_TYPE,AIML_JOB_RUN_ID)
